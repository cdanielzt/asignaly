<template>
    <AppLayout title="Acomodadores">
        ...
        <!-- Schedule list -->
        <div v-if="schedules.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div
            ...
            >
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="font-semibold text-gray-900 text-sm leading-tight">{{ schedule.name }}</p>
                ...
                <div class="flex items-center gap-3 shrink-0">
                    <Link
                        ...
                        class="inline-flex items-center gap-1.5 text-indigo-600 hover:text-indigo-800 text-sm font-medium transition-colors"
                    >
                        Ver
                    ...
                    <RowActions>
                        <button type="button" class="menu-item-danger" @click="destroySchedule(schedule)">Eliminar</button>
                    </RowActions>
                </div>
            </div>
        </div>
...
function destroySchedule(schedule) {
    if (confirm(`¿Eliminar el programa de ${schedule.name}? Se perderán todas las asignaciones. Esta acción no se puede deshacer.`)) {
