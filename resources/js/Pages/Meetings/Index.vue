    ...
    <AppLayout title="Reuniones">
        <div class="flex items-center justify-between mb-6">
            <p class="text-sm text-gray-500">
                {{ meetings.length }} programa{{ meetings.length !== 1 ? 's' : '' }} en total
            </p>
            <button
                type="button"
        ...
        <div v-else class="bg-white rounded-xl shadow-sm border border-gray-100 py-20 text-center">
            ...
            <p class="text-gray-400 text-sm mt-1 mb-6">Crea tu primer programa para comenzar.</p>
            <button
                type="button"
                class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-4 py-2.5 rounded-lg transition-colors"
                @click="openCreate"
        ...
        <!-- Create modal -->
        <Modal :show="formOpen" title="Crear programa de reunión" @close="formOpen = false">
            <p class="text-sm text-gray-500 mb-5 -mt-1">Elige un mes y año para crear el programa de reunión entre semana.</p>
...
function submitForm() {
    form.post('/meetings', { onSuccess: () => (formOpen.value = false) });
}
